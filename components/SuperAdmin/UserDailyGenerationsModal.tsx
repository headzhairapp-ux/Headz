...
import { getUserDailyGenerations, UserDailyGeneration } from '../../services/supabaseService';
import type { UserWithAnalytics } from './UserTable';

interface UserDailyGenerationsModalProps {
  user: UserWithAnalytics;
  onClose: () => void;
}

const UserDailyGenerationsModal: React.FC<UserDailyGenerationsModalProps> = ({ user, onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const data = await getUserDailyGenerations(user.id);
...
export default UserDailyGenerationsModal;
export type { UserDailyGenerationsModalProps };
