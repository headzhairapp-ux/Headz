...
const AuthModal: React.FC<AuthModalProps> = ({
  ...
  // Decode JWT credential from GoogleLogin component
  ...
  // Google OAuth - Handle credential from GoogleLogin component
  const handleGoogleCredentialSuccess = async (credentialResponse: CredentialResponse) => {
    setError(null);
    setIsLoading(true);
    ...
    try {
      ...
      // Decode JWT to get user info, then use it as if it were from the userinfo API
      const decoded = decodeJwtPayload(credentialResponse.credential);
      const accessTokenFallback = credentialResponse.credential;

      // Call authenticateWithGoogle with the credential - we'll also pass decoded info
